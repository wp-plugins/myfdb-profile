<?php
class MyFDB_Companies {
	const WP_Source = 'myfdb_companie_c_';
	/*
	 * name: Name of the company
	 * profile_url: Url to the company profile page on myfdb.com
	 * history: The history of the company truncated to 275 characters
	 * history_url: Url to the company details page on MyFDB.com
	...
	function MyFDB_Companies($IDTagName = 0) {
	...
	function read($IDTagName = 0, $from_MyFDB = false) {
		...
		if ($IDTagName>0) {
			...
			if (!($this->isSaved($IDTagName) && $this->isCached($IDTagName) /*&& $this->isCacheON()*/) || $from_MyFDB) {
				...
				$result = @file_get_contents('http://www.myfdb.com/companies/'.$IDTagName.'/summary.json');
				...
				if($result !== false) {
					...
					$result->history = strip_tags($result->history); //
					if (strlen($result->history)>0 && $result->history[strlen($result->history)-1] != '.') $result->history .= '...';
					$this->About = $result->history; //
					$this->AboutURL = $result->history_url; //
					$this->TotalCredits = $result->total_credits_count; //
